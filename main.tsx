...
const registerServiceWorker = async (): Promise<void> => {
  ...
  if ('serviceWorker' in navigator && 'PushManager' in window) {
    try {
      if (import.meta.env.DEV) console.log('[PWA] Registering service worker...');
      ...
      registration.addEventListener('updatefound', () => {
        ...
        if (newWorker) {
          if (import.meta.env.DEV) console.log('[PWA] New service worker found, installing...');
          
          newWorker.addEventListener('statechange', () => {
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
              if (import.meta.env.DEV) console.log('[PWA] New content available, will refresh on next visit');
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        if (import.meta.env.DEV) console.log('[PWA] Controller changed, reloading page...');
        window.location.reload();
      });
      ...
      if (import.meta.env.DEV) console.log('[PWA] Service worker registered successfully:', registration.scope);
...
const setupPWAInstallPrompt = (): void => {
  ...
  window.addEventListener('appinstalled', () => {
    if (import.meta.env.DEV) console.log('[PWA] App successfully installed');
    deferredPrompt = null;
...
const setupOfflineMonitoring = (): void => {
  const updateOnlineStatus = () => {
    ...
    if (import.meta.env.DEV) console.log(`[PWA] ${isOnline ? 'Online' : 'Offline'} status detected`);
...
const initializeApp = async (): Promise<void> => {
  try {
    ...
    if (rootElement) {
      ...
      if (import.meta.env.DEV) console.log('[PWA] MangaVerse app initialized successfully');
    } else {
