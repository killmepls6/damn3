...
export default function AdminMonetization() {
  ...
  const createAssignmentMutation = useMutation({
    mutationFn: async (data: any) => {
      const csrfToken = await fetchCsrfToken();
      const response = await fetch("/api/admin/manual-assignments", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "X-CSRF-Token": csrfToken,
  ...
  const revokeAssignmentMutation = useMutation({
    mutationFn: async (id: string) => {
      const csrfToken = await fetchCsrfToken();
      const response = await fetch(`/api/admin/manual-assignments/${id}`, {
        method: "DELETE",
        headers: {
          "X-CSRF-Token": csrfToken,
        },
  ...
  const reconcilePurchaseMutation = useMutation({
    mutationFn: async (purchaseId: string) => {
      const csrfToken = await fetchCsrfToken();
      const response = await fetch(`/api/admin/purchases/${purchaseId}/reconcile`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "X-CSRF-Token": csrfToken,
    ...
    onSuccess: () => {
      ...
      toast({ title: "Success", description: "Purchase reconciled successfully" });
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to reconcile purchase", variant: "destructive" });
    }
  });
  ...
  const createCurrencyPackageMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Success", description: "Currency package created successfully" });
      setShowCurrencyPackageDialog(false);
      resetCurrencyPackageForm();
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to create currency package", variant: "destructive" });
    }
  });
  ...
  const updateCurrencyPackageMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const csrfToken = await fetchCsrfToken();
      const response = await fetch(`/api/currency/packages/${id}`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          "X-CSRF-Token": csrfToken,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-packages"] });
      toast({ title: "Success", description: "Currency package updated successfully" });
      setShowCurrencyPackageDialog(false);
      setEditingCurrencyPackage(null);
      resetCurrencyPackageForm();
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to update currency package", variant: "destructive" });
    }
  });
  ...
  const deleteCurrencyPackageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-packages"] });
      toast({ title: "Success", description: "Currency package deleted successfully" });
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to delete currency package", variant: "destructive" });
    }
  });
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Complete revenue and monetization management</p>
            </div>
