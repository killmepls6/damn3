...
export interface IStorage {
  ...
  searchSeries(query: string, filters?: { genre?: string; status?: string; type?: string }): Promise<Series[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async searchSeries(query: string, filters?: { genre?: string; status?: string; type?: string }): Promise<Series[]> {
  ...
  async checkUserChapterAccess(userId: string, chapterId: string): Promise<{ canAccess: boolean; accessType: string; unlockCost?: number; isUnlocked?: boolean; reason?: string }> {
    try {
